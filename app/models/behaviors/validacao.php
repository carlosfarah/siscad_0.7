...
class ValidacaoBehavior extends ModelBehavior {
	...
	function _cpf($data) {
      for( $i = 0; $i < 10; $i++ ){ 
              if ( $data ==  str_repeat( $i , 11) or !preg_match("@^[0-9]{11}$@", $data ) or $data == "12345678909" )return false;                
              if ( $i < 9 ) $soma[]  = $data{$i} * ( 10 - $i ); 
                      $soma2[] = $data{$i} * ( 11 - $i );                       
      } 
      if(((array_sum($soma)% 11) < 2 ? 0 : 11 - ( array_sum($soma)  % 11 )) != $data{9})return false; 
      return ((( array_sum($soma2)% 11 ) < 2 ? 0 : 11 - ( array_sum($soma2) % 11 )) != $data{10}) ? false : true; 
      }
